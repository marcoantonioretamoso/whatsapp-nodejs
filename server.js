...
//         // Verificar si ya existe una instancia activa (conectada o con QR) para este token
//         const activeInstance = await instanceManager.getActiveInstance(token);
...
//                     user: activeInstance.userInfo
//                 });
//             } else if (activeInstance.status === 'qr_generated') {
//                 // Si ya hay un QR generado, devolverlo
//                 return res.json({
//                     qr: activeInstance.qr,
//                     connected: false,
//                     instance_id: activeInstance.instance_id,
//                     status: 'qr_generated',
//                     message: "QR ya generado, escanea para conectar"
//                 });
//             } else if (activeInstance.status === 'connecting') {
//                 // Si está conectando, devolver que espere
//                 return res.status(202).json({
//                     message: "La instancia se está conectando, intenta de nuevo en unos segundos",
//                     connected: false,
//                     instance_id: activeInstance.instance_id,
//                     status: 'connecting'
//                 });
...
// Endpoint para obtener todos los mensajes de un token
app.get("/messages", async (req, res) => {
    ...
    try {
        ...
        // Diagnóstico: Verificar usuario e instancias
        await instanceManager.debugUserAndInstances(token);
        const messages = await instanceManager.getUserMessages(token, instance_id, parseInt(limit), parseInt(offset));
